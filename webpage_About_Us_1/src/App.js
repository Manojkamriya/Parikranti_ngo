import Navbar from "./components/navbar/navbar";
import image from "./image.png";
import "./App.css";
import Admin_div from "./components/admin_div/admin_div";
import Core_team_member from "./components/core_team_member_div/core_team_member";
function App() {
  return (
    <>
      <div className="main_div">
        <Navbar />
        <div className="lower_div">
          <div className="graphic_img">
            {/* <img src={image} alt="" /> */}
          </div>
          <div className="info">
            <div className="vision_div">
              <h2 id="h2-1">VISION</h2>
              <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Enim
                ullam dicta, iusto dolores voluptatibus modi porro aliquid ab
                velit reiciendis esse autem. Praesentium vel unde natus quidem
                eaque id veritatis.Lorem ipsum dolor sit amet consectetur
                adipisicing elit. Enim ullam dicta, iusto dolores voluptatibus
                modi porro aliquid ab velit reiciendis esse autem. Praesentium
                vel unde natus quidem eaque id veritatis.Lorem ipsum dolor sit
                amet consectetur adipisicing elit. Enim ullam dicta, iusto
                dolores voluptatibus modi porro aliquid ab velit reiciendis esse
                autem. Praesentium vel unde natus quidem eaque id
                veritatis.Lorem ipsum dolor sit amet consectetur adipisicing
                elit. Enim ullam dicta, iusto dolores voluptatibus modi porro
                aliquid ab velit reiciendis esse autem. Praesentium vel unde
                natus quidem eaque id veritatis.
              </p>
              <h2 id="h2-2">MISSION</h2>
              <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Enim
                ullam dicta, iusto dolores voluptatibus modi porro aliquid ab
                velit reiciendis esse autem. Praesentium vel unde natus quidem
                eaque id veritatis.Lorem ipsum dolor sit amet consectetur
                adipisicing elit. Enim ullam dicta, iusto dolores voluptatibus
                modi porro aliquid ab velit reiciendis esse autem. Praesentium
                vel unde natus quidem eaque id veritatis.Lorem ipsum dolor sit
                amet consectetur adipisicing elit. Enim ullam dicta, iusto
                dolores voluptatibus modi porro aliquid ab velit reiciendis esse
                autem. Praesentium vel unde natus quidem eaque id
                veritatis.Lorem ipsum dolor sit amet consectetur adipisicing
                elit. Enim ullam dicta, iusto dolores voluptatibus modi porro
                aliquid ab velit reiciendis esse autem. Praesentium vel unde
                natus quidem eaque id veritatis.
              </p>
              <h2 id="h2-3">VALUES</h2>
              <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Enim
                ullam dicta, iusto dolores voluptatibus modi porro aliquid ab
                velit reiciendis esse autem. Praesentium vel unde natus quidem
                eaque id veritatis.Lorem ipsum dolor sit amet consectetur
                adipisicing elit. Enim ullam dicta, iusto dolores voluptatibus
                modi porro aliquid ab velit reiciendis esse autem. Praesentium
                vel unde natus quidem eaque id veritatis.Lorem ipsum dolor sit
                amet consectetur adipisicing elit.
              </p>
              <h2 id="contact_us">CONTACT US</h2>
              <div className="contact_us">
                <a href="#">
                  <svg
                    className="icon"
                    xmlns="http://www.w3.org/2000/svg"
                    data-name="Layer 1"
                    viewBox="0 0 24 24"
                    id="facebook"
                  >
                    <path d="M15.12,5.32H17V2.14A26.11,26.11,0,0,0,14.26,2C11.54,2,9.68,3.66,9.68,6.7V9.32H6.61v3.56H9.68V22h3.68V12.88h3.06l.46-3.56H13.36V7.05C13.36,6,13.64,5.32,15.12,5.32Z" />
                  </svg>
                </a>
                <a href="#">
                  <svg
                    className="icon"
                    xmlns="http://www.w3.org/2000/svg"
                    enableBackground="new 0 0 512 512"
                    viewBox="0 0 512 512"
                    id="gmail"
                  >
                    <polygon
                      fill="#020202"
                      points="193 248.9 26 382.6 26 116.4 125.4 195.2"
                    />
                    <polygon
                      fill="#020202"
                      points="122.1 167.1 44.7 105.7 467.3 105.7 389.9 167.1 296.8 240.9 256 272.6 215.2 240.9"
                    />
                    <path
                      fill="#020202"
                      d="M302.8,261.6l180.7,144.7H28.5l180.7-144.7l40.7,31.5c0.2,0.2,0.5,0.3,0.7,0.5c3.3,2.1,7.6,2.1,10.9,0
                c0.2-0.2,0.5-0.3,0.7-0.5L302.8,261.6z"
                    />
                    <polygon
                      fill="#020202"
                      points="486 116.4 486 382.6 319 248.9 386.6 195.2"
                    />
                    <path
                      fill="none"
                      d="M483.5,406.3H28.5l180.7-144.7l40.7,31.5c0.2,0.2,0.5,0.3,0.7,0.5c3.3,2.1,7.6,2.1,10.9,0
                c0.2-0.2,0.5-0.3,0.7-0.5l40.7-31.5L483.5,406.3z"
                    />
                    <polygon
                      fill="none"
                      points="486 116.4 486 382.6 319 248.9 386.6 195.2"
                    />
                    <polygon
                      fill="none"
                      points="193 248.9 26 382.6 26 116.4 125.4 195.2"
                    />
                    <polygon
                      fill="none"
                      points="467.3 105.7 389.9 167.1 296.8 240.9 256 272.6 215.2 240.9 122.1 167.1 44.7 105.7"
                    />
                  </svg>
                </a>
                <a href="#">
                  <svg
                    className="icon"
                    xmlns="http://www.w3.org/2000/svg"
                    data-name="Instagram w/circle"
                    viewBox="0 0 19.2 19.2"
                    id="instagram"
                  >
                    <path d="M13.498 6.651a1.656 1.656 0 0 0-.95-.949 2.766 2.766 0 0 0-.928-.172c-.527-.024-.685-.03-2.02-.03s-1.493.006-2.02.03a2.766 2.766 0 0 0-.929.172 1.656 1.656 0 0 0-.949.95 2.766 2.766 0 0 0-.172.928c-.024.527-.03.685-.03 2.02s.006 1.493.03 2.02a2.766 2.766 0 0 0 .172.929 1.656 1.656 0 0 0 .95.949 2.766 2.766 0 0 0 .928.172c.527.024.685.03 2.02.03s1.493-.006 2.02-.03a2.766 2.766 0 0 0 .929-.172 1.656 1.656 0 0 0 .949-.95 2.766 2.766 0 0 0 .172-.928c.024-.527.03-.685.03-2.02s-.006-1.493-.03-2.02a2.766 2.766 0 0 0-.172-.929zM9.6 12.168A2.568 2.568 0 1 1 12.168 9.6 2.568 2.568 0 0 1 9.6 12.168zm2.669-4.637a.6.6 0 1 1 .6-.6.6.6 0 0 1-.6.6z" />
                    <circle cx="9.6" cy="9.6" r="1.667" />
                    <path d="M9.6 0a9.6 9.6 0 1 0 9.6 9.6A9.6 9.6 0 0 0 9.6 0zm4.97 11.662a3.67 3.67 0 0 1-.233 1.213 2.556 2.556 0 0 1-1.462 1.462 3.67 3.67 0 0 1-1.213.233c-.534.024-.704.03-2.062.03s-1.528-.006-2.062-.03a3.67 3.67 0 0 1-1.213-.233 2.556 2.556 0 0 1-1.462-1.462 3.67 3.67 0 0 1-.233-1.213c-.024-.534-.03-.704-.03-2.062s.006-1.528.03-2.062a3.67 3.67 0 0 1 .233-1.213 2.556 2.556 0 0 1 1.462-1.462 3.67 3.67 0 0 1 1.213-.233c.534-.024.704-.03 2.062-.03s1.528.006 2.062.03a3.67 3.67 0 0 1 1.213.233 2.556 2.556 0 0 1 1.462 1.462 3.67 3.67 0 0 1 .233 1.213c.024.534.03.704.03 2.062s-.006 1.528-.03 2.062z" />
                  </svg>
                </a>
                <a href="#">
                  <svg
                    className="icon"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 32 32"
                    id="youtube"
                  >
                    <g>
                      <path d="M13.83 19.259L19.262 16l-5.432-3.259z" />
                      <path d="M16 0C7.164 0 0 7.163 0 16s7.164 16 16 16 16-7.164 16-16S24.836 0 16 0zm8.688 16.701c0 1.5-.174 3-.174 3s-.17 1.278-.69 1.839c-.661.738-1.401.742-1.741.786-2.432.186-6.083.192-6.083.192s-4.518-.044-5.908-.186c-.387-.077-1.254-.055-1.916-.792-.521-.562-.69-1.839-.69-1.839s-.174-1.499-.174-3v-1.406c0-1.5.174-2.999.174-2.999s.17-1.278.69-1.841c.661-.739 1.401-.743 1.741-.785 2.431-.188 6.079-.188 6.079-.188h.008s3.648 0 6.079.188c.339.042 1.08.046 1.741.784.521.563.69 1.841.69 1.841s.174 1.5.174 3v1.406z" />
                    </g>
                  </svg>
                </a>
              </div>
            </div>
            <div class="team_member_div">
             <div class="admin_div">
              <Admin_div/>
              <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Aliquam eaque exercitationem voluptatem, aspernatur fugiat eveniet alias aperiam corporis reprehenderit ratione culpa debitis sed quasi consectetur nam esse. Eius, eligendi quia!Lorem ipsum dolor sit amet consectetur adipisicing elit. Aliquam eaque exercitationem voluptatem, aspernatur fugiat eveniet alias aperiam corporis reprehenderit ratione culpa debitis sed quasi consectetur nam esse. Eius, eligendi quia!Lorem ipsum dolor sit amet consectetur adipisicing elit. Aliquam eaque exercitationem voluptatem, aspernatur fugiat eveniet alias aperiam corporis reprehenderit ratione culpa debitis sed quasi consectetur nam esse. Eius, eligendi quia!Lorem ipsum dolor sit amet consectetur adipisicing elit. Aliquam eaque exercitationem voluptatem, aspernatur fugiat eveniet alias aperiam corporis reprehenderit ratione culpa debitis sed quasi consectetur nam esse. Eius, eligendi quia!</p>
             </div>
             <div class="core_team_member">
              <Core_team_member/>
              <Core_team_member/>
              <Core_team_member/>
              <Core_team_member/>
              <Core_team_member/>
              <Core_team_member/>
              <Core_team_member/>
              <Core_team_member/>
             </div>
          </div>
          </div>
          

        </div>
      </div>
    </>
  );
}

export default App;
